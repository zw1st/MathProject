...
class FletcherReeves(Optimizer):
    """Метод сопряжённых градиентов Флетчера-Ривза (устойчивая версия)"""
    ...
    def optimize(self,
        ...
        history = {'k': [], 'x': [], 'f': [], 'grad_norm': [], 'alpha': [], 'd_norm': []}
        ...
        for k in range(self.max_iterations):
            ...
            # 🔹 2. Нормировка направления (стабилизация dot-произведений в овраге)
            ...
            # 🔹 3. Поиск шага (c2=0.1 — стандарт для CG)
            alpha = None
            n_f_ls, n_g_ls = 0, 0
            try:
            ...
            # 🔹 5. Коэффициент Флетчера-Ривза
            denom = np.dot(g, g)
            beta = np.dot(g_new, g_new) / denom if denom > 1e-16 else 0.0

            # 🔹 6. Рестарт: Периодический + Условие Пауэлла (потеря ортогональности)
        ...
        if self.verbose:
            self._print_footer(OptimizationResult(
                ...
                history=history, converged=converged, method_name="Флетчер-Ривз"
            ))
